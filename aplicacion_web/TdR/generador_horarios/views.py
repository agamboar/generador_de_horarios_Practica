...
def get_secciones_disponibles(request, codigo):
    ...
    if request.method == "GET":
        ...
        try:
            secciones_disponibles = nodo_seccion.objects.filter(to_nodo_asignatura__to_user = 19,to_nodo_asignatura__to_asignatura_real__codigo=codigo).values('to_seccion__cod_seccion','to_seccion__num_seccion','to_seccion__vacantes_libres','to_seccion__evento__profesor','to_seccion__evento__dia','to_seccion__evento__modulo','to_seccion__evento__tipo').distinct()  
        except:
            pass
