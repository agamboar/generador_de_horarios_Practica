    ...
    for i in range(show_options):
        ...
        if prev_solution == arr_aux_delete:  # sirve para no mostrar siempre las mismas soluciones
            ...
            for elem in arr_aux_delete:  # muestra las secciones a tomar
                solucion.append(
                    {'nombre': G.nodes[elem[0]]["nombre"],'horario':G.nodes[elem[0]]["horario"], 'nro_seccion': G.nodes[elem[0]]["nro_seccion"], 'cod_asignatura_real': G.nodes[elem[0]]["cod_asignatura_real"], 'eventos': G.nodes[elem[0]]["eventos"], 'cod_seccion': G.nodes[elem[0]]["cod_seccion"]})
